...
apparent_place_saturn <- function(con, jd)
{
  ...
  # Extract the barycentric position and velocity of Saturn
  saturn_ssb <- position_saturn_ssb(con, t)
  ...
  saturn_ssb_au <- saturn_ssb[,1] / KM2AU
  saturn_ssb_au_day <- saturn_ssb[,2] / KM2AU
  
  # Calculate the geometric distance between the positions of the center of mass
  # of Saturn and the Earth
  tmp <- saturn_ssb_au - earth_ssb_au
  d <- magnitude(tmp)
  
  # Calculate a first approximation to the light-travel time between Saturn
  # and the Earth
  tau = d / CAUD
  tt <- t - tau
  
  # Extract the barycentric position of Saturn and the Sun at t - tau
  saturn_ssb <- position_saturn_ssb(con, tt)
  ...
  U <- saturn_ssb_au - earth_ssb_au
  Q <- saturn_ssb_au - sun_ssb_au
  U_mag <- magnitude(U)
  Q_mag <- magnitude(Q)
  ...
  repeat {
    ...
    if ((tau - tau_prime) < 1E-9){
      ...
      saturn_ssb <- position_saturn_ssb(con, (t - tau_prime))
