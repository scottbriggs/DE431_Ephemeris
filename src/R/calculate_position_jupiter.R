...
# Calculate the position and velocity of Jupiter

calculate_position_jupiter <- function(con, jd)
{
  ...
  str1 <- paste("SELECT Julian_Day_Start FROM Jupiter WHERE Julian_Day_Start <=",
                jd,
                "AND Julian_Day_End >=",
                jd,
                "AND Interval = 1")
  ...
  str2 <- paste("SELECT X1, X2, X3, X4, X5, X6, X7, X8,",
