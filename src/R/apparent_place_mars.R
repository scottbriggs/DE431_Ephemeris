apparent_place_mars <- function(con, jd)
{
  ...
  # Extract the barycentric position and velocity of Mars
  mars_ssb <- position_mars_ssb(con, t)
  ...
  mars_ssb_au <- mars_ssb[,1] / KM2AU
  mars_ssb_au_day <- mars_ssb[,2] / KM2AU
  
  # Calculate the geometric distance between the positions of the center of mass
  # of Mars and the Earth
  tmp <- mars_ssb_au - earth_ssb_au
  d <- magnitude(tmp)
  
  # Calculate a first approximation to the light-travel time between Mars
  # and the Earth
  tau = d / CAUD
  tt <- t - tau
  
  # Extract the barycentric position of Mars and the Sun at t - tau
  mars_ssb <- position_mars_ssb(con, tt)
  ...
  U <- mars_ssb_au - earth_ssb_au
  Q <- mars_ssb_au - sun_ssb_au
  U_mag <- magnitude(U)
  Q_mag <- magnitude(Q)
  ...
  repeat {
    ...
    if ((tau - tau_prime) < 1E-9){
      ...
      mars_ssb <- position_mars_ssb(con, (t - tau_prime))
