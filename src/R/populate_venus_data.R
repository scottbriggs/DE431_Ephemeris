...
populate_venus_data <- function(filename, db_conn)
{
  # Read ascii data
  DT <- readRDS(here("data", "processed", filename))
  
  # Column names for Venus
  ...
  temp = matrix(0.0,nrow=22830,ncol=45)
  ...
  dbAppendTable(db_conn, "Venus", venus, row.names = NULL)
}