...
calculate_position_earth <- function(con, jd)
{
  # Get the position and velocity of the earth-moon barycenter and moon
  emb_pos_vel <- calculate_position_emb(con, jd)
  moon_pos_vel <- calculate_position_moon(con, jd)
  
  # Get the value of the earth-moon mass ration (EMRAT)
  query <- "SELECT Header_Value FROM Header WHERE Header_Name = 'EMRAT'"
  emrat <- dbGetQuery(con, query)
