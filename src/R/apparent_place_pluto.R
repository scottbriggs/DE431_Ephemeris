...
apparent_place_pluto <- function(con, jd)
{
  ...
  # Extract the barycentric position and velocity of Pluto
  pluto_ssb <- position_pluto_ssb(con, t)
  ...
  pluto_ssb_au <- pluto_ssb[,1] / KM2AU
  pluto_ssb_au_day <- pluto_ssb[,2] / KM2AU
  
  # Calculate the geometric distance between the positions of the center of mass
  # of Pluto and the Earth
  tmp <- pluto_ssb_au - earth_ssb_au
  d <- magnitude(tmp)
  
  # Calculate a first approximation to the light-travel time between Pluto
  # and the Earth
  tau = d / CAUD
  tt <- t - tau
  
  # Extract the barycentric position of Pluto and the Sun at t - tau
  pluto_ssb <- position_pluto_ssb(con, tt)
  ...
  U <- pluto_ssb_au - earth_ssb_au
  Q <- pluto_ssb_au - sun_ssb_au
  U_mag <- magnitude(U)
  Q_mag <- magnitude(Q)
  ...
  repeat {
    ...
    if ((tau - tau_prime) < 1E-9){
      ...
      pluto_ssb <- position_pluto_ssb(con, (t - tau_prime))
