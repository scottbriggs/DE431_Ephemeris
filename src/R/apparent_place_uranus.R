...
apparent_place_uranus <- function(con, jd)
{
  ...
  # Extract the barycentric position and velocity of Uranus
  uranus_ssb <- position_uranus_ssb(con, t)
  ...
  uranus_ssb_au <- uranus_ssb[,1] / KM2AU
  uranus_ssb_au_day <- uranus_ssb[,2] / KM2AU
  
  # Calculate the geometric distance between the positions of the center of mass
  # of Uranus and the Earth
  tmp <- uranus_ssb_au - earth_ssb_au
  d <- magnitude(tmp)
  
  # Calculate a first approximation to the light-travel time between Uranus
  # and the Earth
  tau = d / CAUD
  tt <- t - tau
  
  # Extract the barycentric position of Uranus and the Sun at t - tau
  uranus_ssb <- position_uranus_ssb(con, tt)
  ...
  U <- uranus_ssb_au - earth_ssb_au
  Q <- uranus_ssb_au - sun_ssb_au
  U_mag <- magnitude(U)
  Q_mag <- magnitude(Q)
  ...
  repeat {
    ...
    if ((tau - tau_prime) < 1E-9){
      ...
      uranus_ssb <- position_uranus_ssb(con, (t - tau_prime))
