...
# Calculate the position and velocity of the Earth-Moon Barycenter

calculate_position_emb <- function(con, jd)
{
  ...
  str1 <- paste("SELECT Julian_Day_Start FROM EMB WHERE Julian_Day_Start <=",
                jd,
                "AND Julian_Day_End >=",
                jd,
                "AND Interval = 1")
  ...
  str2 <- paste("SELECT X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13,",
